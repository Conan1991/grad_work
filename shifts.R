...
calculate_Shifts = function(iteration = 0)
{
  ...
  for (t in 2:number_of_iterations) {
    ...
    #if (t < 10 || t > number_of_iterations - 1 )
    #   print(shiftU[2,])
    ...
    for (k in 2:(cols - 1))
    {
      SHIFTS_U[[t]][1, k] = lambda * step_x / (2 * step_y) * (SHIFTS_V[[t - 1]][1, k + 1] - SHIFTS_V[[t - 1]][1, k - 1]) / (lambda + 2 * nu) - alpha * step_x * x_tr[1] / (lambda + 2 * nu) * (3 * lambda + 2 * nu) + SHIFTS_U[[t]][2, k]
...
(2 * step_y) * (SHIFTS_V[[t - 1]][length(x_tr), k + 1] - SHIFTS_V[[t - 1]][length(x_tr), k - 1]) / (lambda + 2 * nu) + alpha * step_x * x_tr[length(x_tr)] / (lambda + 2 * nu) * (3 * lambda + 2 * nu)
  ...
  for (t in 2:number_of_iterations) {
    for(i in 2:(rows-1))
    {
      for(j in 2:(cols-1))
      {
        ...
        sigma11[i,j] = lambda * (ksi11[i,j] + ksi22[i,j] - 3 * alpha * x_tr[i]) + 2 * nu * (ksi11[i,j] - alpha * x_tr[i])
        sigma12[i,j] = sigma21[i,j] = 2 * nu * ksi12[i,j]
        sigma22[i,j] = lambda * (ksi11[i,j] + ksi22[i,j] - 3 * alpha * x_tr[i]) + 2 * nu * (ksi22[i,j] - alpha * x_tr[i])
        sigma33[i,j]= lambda * (ksi11[i,j] + ksi22[i,j] - 3 * alpha * x_tr[i]) - 2 * nu * alpha * x_tr[i]
      }
    }
  }
